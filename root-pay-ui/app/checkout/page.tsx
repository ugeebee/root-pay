...
// Wrapper component to handle useSearchParams strictly inside a Suspense boundary
function CheckoutGateway() {
  const searchParams = useSearchParams();
  const router = useRouter();
  const clientKey = searchParams.get('client_key');

  const [paymentData, setPaymentData] = useState<{ serverKey: string; upiDeeplink: string } | null>(null);
  ...
  useEffect(() => {
    ...
    // Retrieve the backend keys generated by the previous page
  ...
  useEffect(() => {
    ...
    console.log(`📡 Opening SSE channel for server_key: ${paymentData.serverKey}`);
    const eventSource = new EventSource(`${BACKEND_URL}/tips/stream?server_key=${paymentData.serverKey}`);
    ...
    eventSource.onmessage = (event) => {
      try {
        ...
        if (data.status === 'PAID') {
          ...
          sessionStorage.removeItem('rootpay_gateway'); // Clean up memory
          eventSource.close(); 
        }
      } catch (err) {
...
// Ensure Suspense wraps the component for Next.js build requirements
